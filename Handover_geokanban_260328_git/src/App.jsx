...
// Optimized Lazy Loading for Tabs
...
function ActivityDashboard({ onSignOut }) {
  const { vesselPositions, geofences, activities, vessels, companyVesselIds, crewVesselId } = useData();
  ...
  // Stato Periodo per Contatori Sincronizzati
  const now = new Date();
  ...
  // Calcolo Contatori AIS & Sottomissioni per Periodo
  ...
  const totalNewSubmitted = (activities || [])
    ...
    <div className="min-h-screen bg-[#f7f9fb] font-manrope text-on-surface selection:bg-primary/20">
        ...
        <div className="flex items-center gap-4 lg:gap-6">
          ...
          <div className="flex flex-col">
            <h1 className="font-manrope font-extrabold text-xl lg:text-2xl text-on-surface tracking-tight leading-none mb-1">
              GeoKanban <span className="text-[10px] font-bold text-primary/40 align-top ml-1">v3.15</span>
            </h1>
      ...
      <main className="pt-24 sm:pt-28 lg:pt-36 px-6 sm:px-10 lg:px-16 max-w-[2100px] mx-auto pb-12">
        ...
        <nav className="bg-white/50 backdrop-blur-md rounded-[2.5rem] p-2 mb-8 sm:mb-12 border border-white flex flex-wrap items-center gap-1 shadow-sm overflow-x-auto scrollbar-hide">
          {[
            { id: 'activity', label: 'Vessel Activity', icon: Activity, subLabel: aisTotal > 0 ? `${aisTotal}` : null },
            { id: 'logbook-entry', label: 'Submitted Activity', icon: Edit3, subLabel: aisTotal > 0 ? `${aisSubmitted}/${aisTotal}` : null, permission: perms.submitLogbook || perms.approveLogbook },
            ...
            <button
              ...
              className={`
                flex flex-col items-center justify-center min-w-[120px] lg:min-w-[160px] px-4 lg:px-6 py-3 lg:py-4 rounded-[2rem] tracking-tight transition-all duration-300 whitespace-nowrap
                ${activeTab === item.id 
            ...
            >
              ...
              {item.subLabel && (
                <span className={`text-[10px] font-black mt-1 opacity-60 ${activeTab === item.id ? 'text-white' : 'text-primary'}`}>
                    {item.subLabel}
                </span>
              )}
              {item.id === 'activity' && totalMsgs > 0 && (
                <span className="absolute -top-1 -right-1 bg-secondary text-white text-[9px] px-1.5 py-0.5 rounded-full shadow-sm">{totalMsgs}</span>
              )}
              {item.id === 'logbook-entry' && activeTab !== 'logbook-entry' && totalNewSubmitted > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[9px] px-1.5 py-0.5 rounded-full animate-pulse shadow-sm">{totalNewSubmitted}</span>
              )}
            </button>
          ))}
        </nav>
