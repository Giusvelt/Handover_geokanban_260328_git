...
export default function LogbookEntryModal({ activity, profile, entryMeta, onClose, onSaved }) {
    ...
    const [form, setForm] = useState({
        ...
        // Operational Times (Phase 29)
        commence_time: fmt(activity?.commence_time),
        complete_time: fmt(activity?.complete_time),
        // Narrative
    ...
    const handleSave = async (submit = false) => {
        ...
        try {
            ...
            if (form.commence_time) vaUpdates.commence_time = new Date(form.commence_time).toISOString();
            if (form.complete_time) vaUpdates.complete_time = new Date(form.complete_time).toISOString();
                ...
                <div className="lem-body">
                    ...
                    {needsCargo && (
                        <section className="lem-section lem-highlight-cargo">
                            ...
                            {/* Operational Times (Commence/Complete) */}
                            <div className="lem-grid-2" style={{ marginTop: '16px', borderTop: '1px solid #e2e8f0', paddingTop: '16px' }}>
                                <TimeInput 
                                    label="Commence Loading" 
                                    value={form.commence_time} 
                                    onChange={v => set('commence_time', v)} 
                                    disabled={isSubmitted} 
                                    baseDate={activity?.startTime} 
                                />
                                <TimeInput 
                                    label="Complete Loading" 
                                    value={form.complete_time} 
                                    onChange={v => set('complete_time', v)} 
                                    disabled={isSubmitted} 
                                    baseDate={activity?.endTime || activity?.startTime} 
                                />
                            </div>
