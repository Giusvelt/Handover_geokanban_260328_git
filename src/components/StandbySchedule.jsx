    ...
    const renderCalendarDays = () => {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            blks.push(
                <div
                    key={d}
                    className={`cal-day ${isToday ? 'today' : ''} ${isPast ? 'past' : ''} ${data ? 'has-event' : ''} ${perms.editSchedule && !isPast ? 'clickable group relative' : ''}`}
                    onClick={() => handleDayClick(d)}
                >
                    <div className="flex justify-between items-start w-full">
                        <span className="day-num font-black text-[10px]">{d}</span>
                        {perms.editSchedule && !isPast && (
                            <div className="w-8 h-8 flex items-center justify-center text-white bg-primary rounded-xl shadow-lg border-2 border-white/50 hover:scale-110 active:scale-95 transition-all cursor-pointer z-10">
                                <Plus size={16} strokeWidth={3} />
                            </div>
                        )}
                    </div>
                    {data && (
                        <div className="day-event tooltip-trigger" style={{ marginTop: 'auto', width: '100%' }}>
